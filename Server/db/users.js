...
async function createUser({
  ...
  try {
    ...
    const {
      ...
      `
          INSERT INTO users(name, username, password, address, email, preorderinfo isadmin)
          VALUES ($1, $2, $3, $4, $5, $6)
          ON CONFLICT (username, email) DO NOTHING
          RETURNING *;
          `,
      [name, username, hashedPassword, address, email, preorderinfo, isadmin]
    );
    return user;
  } catch (error) {
    throw error;
...
async function getUserById(id) {
  try {
    const {
      ...
      SELECT id, username, address, email, name, preorderinfo, isadmin
      FROM users
      WHERE id=${id}`);
